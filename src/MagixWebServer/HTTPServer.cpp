...
int HTTPServer::run(IHTTPConfig *conf, IHTTPServerStatusHandler *statusHandler)
{
	...
	do
	{
		...
		*/
		...
		LOGGER_CINFO(theLogger, _T("MagixWebServer 正在运行,根目录[%s],地址[%s:%d],最大连接数[%d].\r\n"), 
	...
	*/
	doStop();
	LOGGER_CWARNING(theLogger, _T("MagixWebServer 启动失败.\r\n"));
	return ret;
}
...
int HTTPServer::stop()
{
	...
	LOGGER_CINFO(theLogger, _T("MagixWebServer 停止.\r\n"));
	return SE_SUCCESS;
}
